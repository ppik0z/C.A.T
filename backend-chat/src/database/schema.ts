...
export const userSettings = mysqlTable('user_settings', {
  ...
  showNotificationPreview: boolean('showNotificationPreview')
    .notNull()
    .default(true),
...
export const authSessions = mysqlTable(
  'auth_sessions',
  {
    ...
    previousRefreshTokenHash: varchar('previousRefreshTokenHash', {
      length: 64,
    }),
...
// ─── AuthActionTokens ────────────────────────────────────────────────────────
export const authActionTokens = mysqlTable(
  'auth_action_tokens',
  {
    id: varchar('id', { length: 36 }).primaryKey(),
    userId: int('userId')
    ...
    targetEmail: varchar('targetEmail', { length: 255 }).notNull(),
    expiresAt: datetime('expiresAt').notNull(),
    consumedAt: datetime('consumedAt'),
    createdAt: datetime('createdAt')
      .notNull()
      .default(sql`CURRENT_TIMESTAMP`),
  },
  (t) => [
    uniqueIndex('uq_auth_action_token_hash').on(t.tokenHash),
    index('idx_auth_action_token_user_purpose').on(t.userId, t.purpose),
    index('idx_auth_action_token_expiry').on(t.expiresAt),
  ],
);
...
export const pushSubscriptions = mysqlTable(
  'push_subscriptions',
  {
    ...
    userId: int('userId')
      .notNull()
      .references(() => users.id, { onDelete: 'cascade' }),
    authSessionId: varchar('authSessionId', { length: 36 })
      .notNull()
...
export const messages = mysqlTable(
  ...
  (t) => [
    ...
    uniqueIndex('uq_message_conversation_index').on(
      t.conversationId,
      t.conversationIndex,
    ),
    index('idx_message_conversation_index').on(
      t.conversationId,
      t.conversationIndex,
    ),
...
export const callSessions = mysqlTable(
  'call_sessions',
  {
    ...
    conversationId: int('conversationId')
      .notNull()
      .references(() => conversations.id, { onDelete: 'cascade' }),
    startedByUserId: int('startedByUserId')
      .notNull()
      .references(() => users.id),
  ...
  (t) => [
    index('idx_call_sessions_conversation_status').on(
      t.conversationId,
      t.status,
    ),
...
export const callParticipants = mysqlTable(
  'call_participants',
  {
    ...
    mediaStatus: varchar('mediaStatus', { length: 30 })
      .notNull()
      .default('idle'),
  ...
  (t) => [
    uniqueIndex('uq_call_participants_session_user').on(
      t.callSessionId,
      t.userId,
    ),
...
export const authActionTokensRelations = relations(
  authActionTokens,
  ({ one }) => ({
    user: one(users, {
...
export const pushSubscriptionsRelations = relations(
  ...
  ({ one }) => ({
    ...
    authSession: one(authSessions, {
      fields: [pushSubscriptions.authSessionId],
...
export const callSessionsRelations = relations(
  callSessions,
  ({ one, many }) => ({
    conversation: one(conversations, {
...
export const callParticipantsRelations = relations(
  ...
  ({ one }) => ({
    callSession: one(callSessions, {
      fields: [callParticipants.callSessionId],
      references: [callSessions.id],
    }),
    user: one(users, {
